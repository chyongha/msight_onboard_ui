        ...
        if path_initialPosition_exists:
            ...
            if path_initialPosition_speed_transmission is not None or path_initialPosition_speed_velocity is not None:
                ...
                if path_initialPosition_speed_transmission is None:
                    ...
                    # NOT a typo, verified against the real ASN.1 schema: the
                    # TransmissionAndSpeed SEQUENCE (used here and in RSAEncoder.py)
                    # really does define this field as 'transmisson' in the actual
                    # SAE J2735 standard — BSMcoreData's OWN transmission field
                    # (used elsewhere in this file) is spelled correctly, so the
                    # spec is just inconsistent with itself. Confirmed by round-
                    # tripping both spellings through the real v2xlib.py: only
                    # 'transmisson' round-trips; 'transmission' raises ASN1ObjErr.
